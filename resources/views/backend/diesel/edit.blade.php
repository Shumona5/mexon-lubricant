...
@endsection
@section('main')
<form action="{{route('diesel.update',$diesel->id)}}" method="post" class=" px-6 py-6 rounded-md space-y-7" enctype="multipart/form-data">
    @csrf
    @method('put')
    <div class="bg-white py-12 rounded-lg shadow-md">
        ...
        <div class="px-10">
            ...
            <div class="mt-1 relative rounded-md shadow-sm">
                <textarea  id="content" name="short_description" class="ckeditor form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:ring-2 focus:border-blue-200 focus:outline-none" placeholder="Description"> {{ $diesel->short_description }} </textarea>
            </div>
        ...
        <div class="px-10">
            ...
            <div class="mt-1 relative rounded-md shadow-sm">
                <textarea  id="content" name="long_description" class="ckeditor form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:ring-2 focus:border-blue-200 focus:outline-none" placeholder="Description"> {{ $diesel->long_description }} </textarea>
            </div>
        ...
        <div class="px-10">
            ...
            <div class="mt-1 relative rounded-md shadow-sm">
                <select required name="status" class="form-select appearance-none
                    ...
                    <option @if($diesel->status=='Active') selected @endif value="Active">Active</option>
                    <option @if($diesel->status=='Inactive') selected @endif value="Inactive">Inactive</option>
        ...
        <div class="mt-8 pt-5">
            <div class="flex justify-end">
                <span class="inline-flex rounded-md shadow-sm">
                    <a href="{{route('diesel.list')}}" class="py-2 px-4 border border-gray-300 rounded-md text-sm leading-5 font-medium text-gray-700 hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800 transition duration-150 ease-in-out">
                        Cancel
                    </a>
                </span>
